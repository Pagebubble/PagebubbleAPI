...
export function show(req, res) {
    return Webpage.findById(req.params.id).exec()
        .then(handleEntityNotFound(res))
        .then(respondWithResult(res))
        .catch(handleError(res));
}

// Creates a new Webpage in the DB
export function create(req, res) {
    return Webpage.create(req.body)
...
export function upsert(req, res) {
    ...
    return Webpage.findOneAndUpdate({_id: req.params.id}, req.body, {new: true, upsert: true, setDefaultsOnInsert: true, runValidators: true}).exec()
        .then(respondWithResult(res))
        .catch(handleError(res));
}

// Updates an existing Webpage in the DB
